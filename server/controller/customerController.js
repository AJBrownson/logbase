...
// Register Customer (without email verification)
  ...
  try {
    ...
    if (!customer.password) {
      return res.send({
        status: false,
        message:
          "To change your password, you need to sign in with your email and password!",
      });
    } else if (
      customer &&
...
// Sign Up with Provider (e.g., Google, Facebook)
  ...
  try {
    ...
    if (isAdded) {
      if (bcrypt.compareSync(user.password, isAdded.password)) {
        ...
        res.status(401).send({
          message:
            "Email already exists, please log in with your email and password!",
        });
      }
    } else {
